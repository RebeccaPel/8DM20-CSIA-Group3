...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        # perform forward pass through first convolutional layer
        x = self.conv1(x)
        # apply batch normalization and ReLU activation
        x = self.bn1(x)
        x = self.relu(x)

        # perform forward pass through second convolutional layer
        x = self.conv2(x)
        # apply batch normalization and ReLU activation
        x = self.bn2(x)
        x = self.relu(x)
...
class Encoder(nn.Module):
    ...
    def __init__(self, spatial_size=(64, 64), z_dim=256, chs=(1, 64, 128, 256)):
        ...
        self.enc_blocks = nn.ModuleList(
            ...
            [Block(chs[i], chs[i + 1]) for i in range(0, len(chs) - 1)]
        )
        # max pooling
        self.pool = nn.MaxPool2d(kernel_size=2)  # try without stride

        # height and width of images at lowest resolution level
        _h, _w = spatial_size[0] // 2 ** 3, spatial_size[1] // 2 ** 3  # TODO
        # _h, _w = 8, 8  # TODO
    ...
    def forward(self, x):
        ...
        for block in self.enc_blocks:
            # TODO: conv block
            x = block(x)  # forward pass to conv block
            # TODO: pooling
            x = self.pool(x)  # pass to max pool
        # TODO: output layer
        x = self.out(x)
...
class Generator(nn.Module):
    ...
    def __init__(self, z_dim=256, chs=(256, 128, 64, 32), spatial_size=[64, 64]):

        super().__init__()
        self.chs = chs

        _h, _w = spatial_size[0] // 2 ** 3, spatial_size[1] // 2 ** 3
        # _h, _w = 8, 8

        self.z_dim = z_dim
        self.proj_z = nn.Linear(
            self.z_dim, self.chs[0] * _h * _w
        )  # fully connected layer on latent space

        self.reshape = lambda x: torch.reshape(
            x, (-1, self.chs[0], _h, _w)
        )  # reshaping
        ...
        for i in range(len(chs) - 1):
            self.upconvs.append(
                nn.ConvTranspose2d(chs[i], chs[i], kernel_size=2, stride=2)  # +1 en 2 arg
            )

        # TODO: conv block
        self.dec_blocks = nn.ModuleList(
            [Block(chs[i], chs[i + 1]) for i in range(len(chs) - 1)]  # *2
        )
    ...
    def forward(self, z):
        ...
        # x = self.proj_o(x)  # TODO: output layer
        # the output layer is in the head of the vae
...
class VAE(nn.Module):
    ...
    def __init__(
        ...
        # Do a tahn for the putput of generator
        self.head = nn.Sequential(
            # nn.Tanh()
            nn.Conv2d(dec_chs[-1], 1, kernel_size=3, padding=1), nn.Tanh()
