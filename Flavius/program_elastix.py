...
if __name__ == "__main__":
    ...
    axarr[0].imshow(fixed_image[30, :, :], cmap='gray')
    axarr[0].imshow(fixed_image_seg[30, :, :], alpha=0.5*(fixed_image_seg[30, :, :]>0), cmap='Purples')
    axarr[0].set_title('Fixed image')
    axarr[1].imshow(moving_image[30, :, :], cmap='gray')
    axarr[1].set_title('Moving image')
    axarr[2].imshow(transformed_bsplined_segmentation, cmap='gray')
    axarr[2].set_title('Resulted B-splined segmentation')
    axarr[3].imshow(transformed_bsplined_image, cmap='gray')
    axarr[3].set_title('Resulted B-splined image')
    axarr[4].imshow(imageio.imread(jacobian_determinant_path.replace('dcm', 'tiff')))
